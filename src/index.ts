...
async function init() {
  ...
  try {
    ...
    for (const file of FILES_TO_DOWNLOAD) {
      console.log(`📥 Downloading ${path.basename(file.url)}...`);
      const response = await fetch(file.url);
      if (!response.ok) throw new Error(`Could not download: ${file.url}`);
      const content = await response.text();

      const targetPath = path.join(process.cwd(), rootPrefix, file.subPath);
      await fs.ensureDir(path.dirname(targetPath));
      await fs.writeFile(targetPath, content);
      console.log(`✅ Saved to ${path.join(rootPrefix, file.subPath)}`);
    }

    const possibleCssPaths = [
      path.join(process.cwd(), 'src', 'app', 'global.css'),
...
`;
      ...
      console.log(
        `✅ Styles added to: ${path.relative(process.cwd(), cssPath)}`
      );
    }

    console.log('📦 Installing dependencies...');
    execSync(
      'npm install date-fns lucide-react framer-motion --no-fund --no-audit',
      { stdio: 'inherit' }
    );
